...
export const extractRoleFromLogHandler = async (
  interaction: CommandInteraction,
) => {
  const { guild, options } = interaction;

  await interaction.reply({
    content: "Extractiong role data from audit log...",
    ephemeral: true,
  });
  ...
  if (log?.size) {
    ...
    if (Object.keys(changeRecord).length) {
      saveToFs(`role-${roleId}`, JSON.stringify(changeRecord, null, 2));

      await interaction.followUp({
        content: "Role extracted",
        ephemeral: true,
      });
    } else {
      await interaction.followUp({
